...
public class Forum {
     ...
     * @return An ArrayList of Comments will have only one entry with a null
     * user if it is an Unauthorized Action.
     */
    ...
    /**
     * Makes an empty ArrayList of comments with a designated string as the
     * first comment with a null user.
     *
    ...
    public void parseAddList(String s) {
        String[] tmp = s.split("\\|");
        for (int i = 0; i < tmp.length; i += 2) {
            comments.add(new Comment(new User(tmp[i], "", false, false), tmp[i + 1]));
        }
    }
}
